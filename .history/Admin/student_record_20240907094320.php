...
if(strlen($_SESSION['login_email'])=="")
  {   
   ...
   if(isset($_POST['generate-btn'])){
    $class_id = $_POST['cmdclass']; // assuming you're posting the selected class id

    $query = "SELECT sat.Surname, student.FirstName, srt.SchoolName 
               FROM studentadmissiontbl sat 
               LEFT JOIN schoolregistrationtbl srt ON sat.SchRegCode = srt.SchRegCode 
               WHERE sat.ClassAT = :class_id";
    $stmt = $dbh->prepare($query);
    $stmt->bindParam(':class_id', $class_id);
    $stmt->execute();
    $result = $stmt->fetchAll();
    
    // do something with the result, e.g., generate the ID cards
    foreach($result as $row){
      // generate ID card for each student
      echo "Generating ID card for " . $row['Surname'] . " " . $row['FirstName'] . " of " . $row['SchoolName'] . "<br>";
    }
  }
