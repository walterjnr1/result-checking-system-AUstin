...
if(strlen($_SESSION['login_email'])=="")
  {   
   ...
   if(isset($_POST['generate-btn'])){
    $indexno=$_GET['id'];
    $data = $dbh->query("SELECT student.Surname, student.FirstName, school.SchoolName 
                         FROM studentadmissiontbl student 
                         LEFT JOIN schoolregistrationtbl school 
                         ON student.SchRegCode  = school.SchRegCode 
                         where student.StuAdmNo ='$indexno'")->fetchAll();   
    foreach ($data as $row) {
        $Surname =$row['Surname'];
        $FirstName =$row['FirstName'];
        $SchoolName=$row['SchoolName'];  
        // You can use these variables now
        echo "Surname: $Surname, FirstName: $FirstName, SchoolName: $SchoolName";
    }
   }
