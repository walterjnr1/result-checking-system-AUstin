...
if(strlen($_SESSION['login_email'])=="")
  {   
   ...
   $indexno=$_GET['id'];
   $data = $dbh->query("SELECT student.*,sat.status as student_status, srt.* FROM studentadmissiontbl sat LEFT JOIN schoolregistrationtbl srt ON sat.SchRegCode  = srt.SchRegCode ORDER BY sat.FirstName DESC ")->fetchAll();   
