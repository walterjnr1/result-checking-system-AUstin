...
if(strlen($_SESSION['login_email'])=="")
  {   
   ...
   //pass classid to bulkIDcard.php page
   $classid = $_POST['classid'];
   $classid = base64_encode($classid);
   header('location:bulkIDcard.php?classid='.$classid);
    ...
    <div class="content">
      ...
      <div class="table-responsive">
        <table id="example1" class="table table-bordered table-striped">
          ...
          <tbody>
		    <?php 
$data = $dbh->query("SELECT student.*,student.status as student_status, 
    school.*, class.* FROM studentadmissiontbl student LEFT JOIN schoolregistrationtbl school ON student.SchRegCode = school.SchRegCode
    LEFT JOIN tblclass class ON student.ClassAT = class.id LEFT JOIN tblclass class ON student.CurClass = class.id ORDER BY student.FirstName DESC")->fetchAll();       
          $cnt=1;
          foreach ($data as $row) {
          ?>
            <tr>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary" id="generate-btn" name="generate-btn">Generate</button>
      </div>
    </div>
  </div>
</div>
