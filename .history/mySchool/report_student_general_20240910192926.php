...
if (empty($_SESSION['login_SchRegCode'])) {
 ...
 $query=$_POST['cmdsearch'];
   ...
   $row_query= $data->fetch();
   $query_class=$row_query['classname'] ;
?>
<!DOCTYPE html>
<html>
...
<title>Student Report by CLass | <?php echo $website_name ;?></title>
...
<body>
  ...
  <div id="wrapper">
    ...
    <nav class="navbar-default navbar-static-side" role="navigation">
        ...
        $data = $dbh->query("SELECT student.*, school.*, class.* 
                             FROM studentadmissiontbl student 
                             LEFT JOIN schoolregistrationtbl school ON student.SchRegCode = school.SchRegCode
                             LEFT JOIN tblclass class ON student.CurClass = class.id 
                             WHERE student.FirstName LIKE '$query' or student.Surname LIKE '$query' or student.otherName LIKE '$query' or student.sex LIKE '$query' or student.StuAdmNo LIKE '$query' or student.parentName LIKE '$query' or student.parentPhone LIKE '$query' or student.parentEmail LIKE '$query' ORDER BY student.FirstName DESC")->fetchAll();
        $cnt=1;
        foreach ($data as $row) {
        ?>
                          <tr class="gradeX">
