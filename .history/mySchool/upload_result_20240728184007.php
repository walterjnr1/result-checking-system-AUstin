...
$sql = "SELECT * FROM tblresultsummary where resultID ='$resultID' or class='$class' and examYear='$examyear' and term='$term' and examType='$examtype'";
...
$query = "SELECT c.classname,e.exam_name,er.* FROM tblclass c INNER JOIN tblresultsummary er ON c.id = er.class INNER JOIN tblexamtype e ON er.examType = e.id";
...
$mail->addAddress($schoolemail,$SchoolName);     //Add a recipient

//Attachment
$mail->addAttachment($_FILES['examcalculator']['tmp_name'], $_FILES['examcalculator']['name']);

$message = "
<html>
<head>
  <title>$SchoolName - $term - $examyear - $classname - $exam_name - Result Upload| $website_name</title>
  <style>
    body {
      padding: 20px;
    }
...
<body>
  <table width='80%' cellpadding='0' cellspacing='0' border='0'>
    ...
    <tr>
      <td class='email-body'>
        <p style='text-align: justify;'>Congratulation $SchoolName,</p>
        <p style='text-align: justify;'>We are happy to inform you that Your result was uploaded successfully for Year: $examyear,Term: $term, Exam Name: $exam_name, Class: $classname .</p>
        <p style='text-align: justify;'>Please do well to generate purchase your scratch card now </p>
        
        <p style='text-align: justify;'>Your MS Excel calculator Template has also been attached in this email. </p>

        <p style='text-align: justify;'><b>For your scratch card and others, you can always reach out to us via Phone Numbers: $phone1,$phone2 and email:$website_email</b>   </p>
...
$mail->Subject = "$SchoolName - $term - $examyear - $classname - $exam_name - Result Upload  | $website_name ";
$mail->Body    = $message;
$mail->send();
}
}
//insert into result summary
$query = "INSERT into `tblresultsummary` (resultID,examYear,term,class,examType)
VALUES ('$resultID','$examyear','$term','$class','$examtype')";
$result = mysqli_query($conn,$query);
if($result){
