...
$data = $dbh->query("SELECT resulttable.*, examtable.* , classtable.* FROM tblresultsummary resulttable ON resulttable.class = classtable.id INNER JOIN examtable ON examtable.id = resulttable.ExamsType where resulttable.SchRegCode='$SchRegCode' ORDER BY resulttable.id DESC ")->fetchAll();
