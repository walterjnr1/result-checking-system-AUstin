...
$data = $dbh->query("SELECT student.*, school.*, class.* 
                     ...
                     WHERE (student.FirstName LIKE '$query' 
                            OR student.Surname LIKE '$query' 
                            OR student.otherName LIKE '$query' 
                            OR student.sex LIKE '$query' 
                            OR student.StuAdmNo LIKE '$query' 
                            OR student.parentName LIKE '$query' 
                            OR student.parentPhone LIKE '$query' 
                            OR student.parentEmail LIKE '$query') 
                     AND school.SchRegCode='$SchRegCode' 
                     ORDER BY student.FirstName DESC")->fetchAll(); $query = '%' . $query . '%'; // add wildcard characters to the query
$data = $dbh->query("SELECT student.*, school.*, class.* 
                     FROM studentadmissiontbl student 
                     LEFT JOIN schoolregistrationtbl school ON student.SchRegCode = school.SchRegCode
                     LEFT JOIN tblclass class ON student.CurClass = class.id 
